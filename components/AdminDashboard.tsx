...
import { Users, UserPlus, Palette, Settings2, ShieldCheck, Activity, MessageCircle, RefreshCw, ChevronRight, Inbox, ClipboardCheck, Cloud, CloudOff, CheckCircle2, AlertCircle, Database, Zap, Cpu, Radio, Shield, Archive, Trash2, Clock, UploadCloud } from 'lucide-react';
...
const AdminDashboard: React.FC<Props> = ({ 
  clients, pendingSummaries, pendingWorkouts, onOpenPortal, onEditPortal, onOpenChat, onArchiveClient, onAddClient, onOpenBranding, fullClients, version
}) => {
  const { cloudSync, cloudError, lastServerUpdate, hasInitialCloudSync } = useApp();
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-top-4 duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <StatCard label="Log Submissions" value={pendingWorkouts.toString()} icon={<Activity className={pendingWorkouts > 0 ? "text-blue-500" : "text-slate-700"} />} highlight={pendingWorkouts > 0} />
        <StatCard label="Phase Summaries" value={pendingSummaries.toString()} icon={<ClipboardCheck className={pendingSummaries > 0 ? "text-amber-500" : "text-slate-700"} />} highlight={pendingSummaries > 0} />
      </div>
        ...
        <div className="grid gap-6">
          {sortedClients.length === 0 ? (
            ...
            sortedClients.map(clientSummary => {
              ...
              const unreadMessages = fullClient?.messages?.filter(m => m.senderId !== 'coach' && !m.isRead).length || 0;
              return (
                <div key={clientSummary.id} className="glass-card p-8 rounded-[2.5rem] border-slate-800/50 flex items-center justify-between group hover:border-blue-500/40 transition-all shadow-xl bg-slate-950/20">
                  <div className="flex items-center gap-8">
                    <div className="w-20 h-20 rounded-3xl bg-slate-900 border-2 border-slate-800 overflow-hidden relative shadow-inner">
                      {clientSummary.avatar ? <img src={clientSummary.avatar} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-700 font-black italic text-2xl">IS</div>}
                      {clientSummary.hasNewSubmission && <div className="absolute top-0 right-0 w-5 h-5 bg-red-500 rounded-full border-[6px] border-[#020617] animate-pulse"></div>}
                    </div>
                    <div>
                      <h4 className="text-3xl font-black text-white brand-font tracking-tight flex items-center gap-4">
                        {clientSummary.name}
                        {clientSummary.hasNewSubmission && <span className="text-[9px] bg-red-500/10 text-red-500 px-4 py-1.5 rounded-full font-black tracking-widest animate-in zoom-in">NEW DATA</span>}
                      </h4>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => onOpenChat(clientSummary.id)} className="p-5 bg-slate-950 rounded-2xl text-slate-600 hover:text-emerald-400 transition-colors relative border border-white/5 group-hover:bg-slate-900 group-hover:border-emerald-500/20" title="Open Chat">
                      <MessageCircle size={24} />
                      {unreadMessages > 0 && <div className="absolute -top-3 -right-3 min-w-[24px] h-[24px] bg-red-500 rounded-full flex items-center justify-center text-[10px] font-black text-white border-4 border-[#020617] shadow-lg">{unreadMessages}</div>}
                    </button>
                    <button onClick={() => onEditPortal(clientSummary.id)} className="p-5 bg-slate-950 rounded-2xl text-slate-600 hover:text-blue-400 transition-colors border border-white/5 group-hover:bg-slate-900 group-hover:border-blue-500/20" title="Edit Protocol"><Settings2 size={24} /></button>
